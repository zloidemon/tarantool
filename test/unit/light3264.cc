...
simple_test()
{
	...
	for(size_t limits = start_limits; limits <= 2 * rounds; limits *= 10) {
		...
		for (size_t i = 0; i < rounds; i++) {
			...
			bool has1 = light_find(&ht, h, val, &itr);
			bool has2 = vect[val];
			assert(has1 == has2);
			if (has1 != has2) {
...
collision_test()
{
	...
	for(size_t limits = start_limits; limits <= 2 * rounds; limits *= 10) {
		...
		for (size_t i = 0; i < rounds; i++) {
			...
			hash_t h = hash(val);
			struct light_iterator itr;
			bool has1 = light_find(&ht, h * 1024, val, &itr);
			bool has2 = vect[val];
			assert(has1 == has2);
			if (has1 != has2) {
				fail("find key failed!", "true");
			...
			for (hash_value_t test = 0; test < limits; test++) {
				if (vect[test]) {
					if (!light_find(&ht, hash(test) * 1024,
						        test, &itr))
						identical = false;
				} else {
					if (light_find(&ht, hash(test) * 1024,
