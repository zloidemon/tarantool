...
do_test where-1.0 {
  execsql {
    CREATE TABLE t1(w int PRIMARY KEY, x int, y int);
    CREATE TABLE t2(p int PRIMARY KEY, q int, r int, s int);
  }
...
do_test where-11.1 {
  execsql {
   CREATE TABLE t99(Dte INT PRIMARY KEY, X INT);
...
do_test where-15.1 {
  execsql {
    ...
    CREATE TEMP TABLE t1 (a PRIMARY key, b, c, d, e);
    CREATE TEMP TABLE t2 (f PRIMARY key);
...
do_test where-17.1 {
  execsql {
    ...
    INSERT INTO tbltmp VALUES(42, 3);
    INSERT INTO tbltmp VALUES(43, 4);
  }
} {}
do_test where-17.2 {
  execsql {
...
do_test where-17.3 {
  execsql {
    SELECT a.id, (SELECT b.id FROM tbltmp AS b WHERE b.id>a.id)
    FROM tbltmp AS a
    WHERE a.eventtype=3;
  }
} {42 43}
...
do_execsql_test where-18.1 {
  CREATE TABLE t181(id int primary key, a);
  CREATE TABLE t182(id int primary key, b,c);
  INSERT INTO t181 VALUES(0, 1);
