...
do_eqp_test 1.2 {
  ...
  0 0 1 {SEARCH TABLE t1 USING COVERING INDEX 517_1_i1 (a=?)} 
  0 0 1 {SEARCH TABLE t1 USING COVERING INDEX 517_2_i2 (b=?)} 
  0 1 0 {SCAN TABLE t2}
}
do_eqp_test 1.3 {
  ...
  0 0 0 {SCAN TABLE t2}
  0 1 1 {SEARCH TABLE t1 USING COVERING INDEX 517_1_i1 (a=?)} 
  0 1 1 {SEARCH TABLE t1 USING COVERING INDEX 517_2_i2 (b=?)} 
}
do_eqp_test 1.3 {
  SELECT a FROM t1 ORDER BY a
} {
  0 0 0 {SCAN TABLE t1 USING COVERING INDEX 517_1_i1}
}
do_eqp_test 1.4 {
  SELECT a FROM t1 ORDER BY +a
} {
...
det 2.2.2 "SELECT DISTINCT min(x), max(x) FROM t2 GROUP BY x ORDER BY 1" {
  0 0 0 {SCAN TABLE t2 USING COVERING INDEX 522_1_t2i1}
  0 0 0 {USE TEMP B-TREE FOR DISTINCT}
  0 0 0 {USE TEMP B-TREE FOR ORDER BY}
}
...
det 2.2.6 "SELECT DISTINCT t2.x FROM t1, t2 ORDER BY t2.x" {
  0 0 1 {SCAN TABLE t2 USING COVERING INDEX 522_1_t2i1}
  0 1 0 {SCAN TABLE t1}
}

det 2.3.1 "SELECT max(x) FROM t2" {
  0 0 0 {SEARCH TABLE t2 USING COVERING INDEX 522_1_t2i1}
}
...
do_eqp_test 3.1.4 {
  ...
  0 0 0 {SCAN TABLE t1}
  0 0 0 {EXECUTE SCALAR SUBQUERY 1}
  1 0 0 {SCAN TABLE t2 USING COVERING INDEX 522_1_t2i1}
}
...
det 3.2.2 {
  ...
  1 0 0 {SCAN TABLE t1} 
  1 0 0 {USE TEMP B-TREE FOR ORDER BY} 
  2 0 0 {SCAN TABLE t2 USING COVERING INDEX 522_1_t2i1} 
...
do_eqp_test 4.2.2 {
  ...
  1 0 0 {SCAN TABLE t1} 
  1 0 0 {USE TEMP B-TREE FOR ORDER BY}
  2 0 0 {SCAN TABLE t2 USING INDEX 522_1_t2i1} 
  0 0 0 {COMPOUND SUBQUERIES 1 AND 2 (UNION ALL)} 
}
do_eqp_test 4.2.3 {
...
do_eqp_test 4.3.2 {
  ...
  2 0 0 {SCAN TABLE t1} 
  3 0 0 {SCAN TABLE t2 USING COVERING INDEX 522_1_t2i1} 
  1 0 0 {COMPOUND SUBQUERIES 2 AND 3 USING TEMP B-TREE (UNION)}
  4 0 0 {SCAN TABLE t1} 
...
# 0|0|0|SCAN TABLE t1
#
do_execsql_test 5.1.0 { CREATE TABLE t1(idt1 PRIMARY KEY, a INT, b INT, ex TEXT) }
...
# 0|1|1|SCAN TABLE t2
#
do_execsql_test 5.4.0 {CREATE TABLE t2(idt2 primary key, c INT, d INT, ex TEXT)}
det 5.4.1 "SELECT t1.a, t2.c FROM t1, t2 WHERE t1.a=1 AND t1.b>2" {
  0 0 0 {SEARCH TABLE t1 USING COVERING INDEX 517_2_i2 (a=? AND b>?)}
  0 1 1 {SCAN TABLE t2}
}
...
det 5.5 "SELECT t1.a, t2.c FROM t2, t1 WHERE t1.a=1 AND t1.b>2" {
  0 0 1 {SEARCH TABLE t1 USING COVERING INDEX 517_2_i2 (a=? AND b>?)}
  0 1 0 {SCAN TABLE t2}
}
