...
do_test triggerC-4.1.1 {
  ...
  execsql {
    ...
    CREATE TRIGGER t4bd BEFORE DELETE ON t4 BEGIN
      INSERT INTO log VALUES(old.a     || ' ' || typeof(old.a)     || ' ' ||
                             old.b     || ' ' || typeof(old.b)     || ' ' ||
                             old.c     || ' ' || typeof(old.c)
      );
    END;
...
do_test triggerC-11.0 {
  catchsql { DROP TABLE IF EXISTS log }
  execsql  { CREATE TABLE log(a PRIMARY KEY, b) }
} {}
