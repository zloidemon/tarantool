...
} {5 31 57 82 83 84 85 86 87 scan 98 sort 0}
do_test where9-1.6 {
  count_steps {
    SELECT a FROM t1
     WHERE b=1012
...
} {1 80 9999 2 80 9999 scan 393 sort 2}
do_test where9-2.8 {
  count_steps {
    SELECT t3.x, t1.a, coalesce(t2.a,9999)
      FROM t3 JOIN
...
} {1 80 2 1 80 28 1 80 54 1 80 80 2 80 2 2 80 28 2 80 54 2 80 80 scan 393 sort 2}
...
do_test where9-9.1 {
  db eval {
    CREATE TABLE t91(x PRIMARY KEY); INSERT INTO t91 VALUES(1);
    ...
    CREATE TEMP TABLE x9 (x PRIMARY KEY, y int, a, b);
    INSERT INTO x9 SELECT * FROM t91 LEFT JOIN t92 ON a=2 OR b=3;
...
do_test where9-10.2 {
  db eval {
    CREATE TABLE t102 (id TEXT PRIMARY KEY NOT NULL);
