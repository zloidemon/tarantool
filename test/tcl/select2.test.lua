...
execsql {CREATE TABLE tbl1(id int, f1 int, f2 int, primary key(id))}
execsql {BEGIN}
for {set i 0} {$i<=30} {incr i} {
  execsql "INSERT INTO tbl1 VALUES([expr {$i}], [expr {$i%9}],[expr {$i%10}])"
}
execsql {COMMIT}
...
ifcapable tclvar {
  do_test select2-2.0.1 {
    set t1 [time {
      execsql {DROP TABLE IF EXISTS tbl2}
      # execsql {CREATE TABLE tbl2(f1 int, f2 int, f3 int); BEGIN;}
      execsql {CREATE TABLE tbl2 (f1 int, f2 int, f3 int, primary key(f1));}
...
do_test select2-2.0.2 {
  set t2 [time {
    execsql {CREATE TABLE tbl2(f1 int primary key, f2 int, f3 int); BEGIN;}
    for {set i 1} {$i<=30000} {incr i} {
      set i2 [expr {$i*2}]
      set i3 [expr {$i*3}]
