...
do_test update-3.1 {
  ...
  for {set i 1} {$i<=10} {incr i} {
    set sql "INSERT INTO test1 VALUES($i,$i,[expr {1<<$i}])"
    execsql $sql
  }
  execsql {SELECT f1,f2 FROM test1 ORDER BY f1}
...
do_test update-5.4.1 {
  execsql {SELECT f1,f2 FROM test1 WHERE f1==78 ORDER BY f1,f2}
} {78 128}
do_test update-5.4.2 {
  execsql {SELECT f1,f2 FROM test1 WHERE f1==778 ORDER BY f1,f2}
} {778 128}
do_test update-5.4.3 {
  execsql {SELECT f1,f2 FROM test1 WHERE f1==8 ORDER BY f1,f2}
...
do_test update-5.5.4 {
  execsql {SELECT f1,f2 FROM test1 WHERE f1==777 ORDER BY f1,f2}
} {777 128}
...
do_test update-5.6.2 {
  execsql {SELECT f1,f2 FROM test1 WHERE f1==77 ORDER BY f1,f2}
} {77 128}
...
do_test update-6.1.2 {
  execsql {SELECT f1,f2 FROM test1 WHERE f2==89 ORDER BY f1,f2}
} {8 89}
do_test update-6.1.3 {
  execsql {SELECT f1,f2 FROM test1 WHERE f1==88 ORDER BY f1,f2}
} {}
do_test update-6.2 {
  execsql {UPDATE test1 SET f2=f2-1 WHERE f1==8 and f2>800}
  execsql {SELECT f1,f2 FROM test1 ORDER BY f1,f2}
...
do_test update-6.3.3 {
  execsql {SELECT f1,f2 FROM test1 WHERE f2==88 ORDER BY f1,f2}
} {8 88}
do_test update-6.4 {
  execsql {UPDATE test1 SET f1=f1+1 WHERE f2==128}
  execsql {SELECT f1,f2 FROM test1 ORDER BY f1,f2}
