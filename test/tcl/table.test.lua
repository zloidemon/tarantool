...
#
do_test table-1.10 {
  execsql {CREATE TABLE "create" (f1 int primary key)}
  #execsql {SELECT name FROM sqlite_master WHERE type!='meta'}
} {}
#} {create}
do_test table-1.11 {
...
do_test table-1.12 {
  execsql {CREATE TABLE test1("f1 ho" int primary key)}
...
#
do_test table-2.1 {
  execsql {CREATE TABLE TEST2(one text primary key)}
  catchsql {CREATE TABLE test2(id primary key, two text default 'hi')}
} {1 {table test2 already exists}}
do_test table-2.1.1 {
  catchsql {CREATE TABLE "test2" (two primary key)}
} {1 {table "test2" already exists}}
# do_test table-2.1b {
...
# } {1 {object name reserved for internal use: sqlite_master}}
do_test table-2.1d {
  catchsql {CREATE TABLE IF NOT EXISTS test2(x primary key,y)}
} {0 {}}
do_test table-2.1e {
...
#
do_test table-2.2a {
  execsql {CREATE TABLE test2(id primary key, one text)}
...
#} \{$big_table\}
do_test table-3.2 {
  set v [catch {execsql {CREATE TABLE BIG(xyz foo primary key)}} msg]
  lappend v $msg
...
do_test table-4.1 {
  for {set i 1} {$i<=100} {incr i} {
    set sql "CREATE TABLE [format test%03d $i] (id primary key, "
  ...
  #execsql {SELECT name FROM sqlite_master WHERE type!='meta' ORDER BY name}
  execsql {SELECT name FROM _space WHERE id>500}
} $r
# do_test table-4.1b {
#   db close
#   sqlite3 db test.db
...
do_test table-5.3 {
  ifcapable {explain} {
    execsql {EXPLAIN CREATE TABLE test1(f1 int primary key)}
  }
  execsql {SELECT name FROM _space WHERE id>500}
  #execsql {SELECT name FROM sqlite_master WHERE type!='meta'}
} {}
...
#
do_test table-5.4 {
  execsql {CREATE TABLE test1(f1 int primary key)}
  ifcapable {explain} {
...
do_test table-14.1 {
  set rc [
      ...
      db eval {SELECT * FROM tablet8 LIMIT 1} {} {
        db eval {CREATE TABLE t9(a primary key, b, c)}
      }
    } msg
  ]
...
do_test table-15.1 {
  ...
  for {set i 0} {$i<2000} {incr i} {
    execsql "CREATE TABLE tbl$i (a primary key, b, c)"
  }
  execsql {COMMIT}
} {}
do_test table-15.2 {
